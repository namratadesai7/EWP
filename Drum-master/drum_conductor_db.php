...
if (isset($_POST['submit'])) {
    ...
    $sql = "INSERT INTO drum_conductor (id,name,isDelete,createdBy) values ('$id','$name','0','$user')";
    $result = sqlsrv_query($conn, $sql);

    if ($result) {
        header('Location: drum_conductor.php');
...
if (isset($_GET['deleteid'])) {
    ...
    $sql = "UPDATE drum_conductor SET isDelete = '1' where id=$id";
    $result = sqlsrv_query($conn, $sql);
...
if (isset($_POST['update'])) {
    ...
    $sql = "UPDATE drum_conductor SET name='$editName', updatedAt='$cur_date', updatedBy='$user' where id='$editId'";
    $result = sqlsrv_query($conn, $sql);
